...
function SectionFaq({ faqs }: { faqs: { question: string, answer: React.ReactNode; }[] }) {
  return (
    <div className="max-w-screen-md mx-auto px-4 py-4 sm:px-6 lg:px-8 flex flex-col justify-between">
      <div className="text-center">
        <p className="mt-4 text-sm leading-7 text-gray-500 font-regular">
          F.A.Q
        </p>
        <h3 className="text-3xl sm:text-4xl leading-normal font-extrabold tracking-tight text-gray-900 dark:text-gray-500">
          ...
          {faqs.map((faq) => (
            <li key={`faq-${faq.question}`} className="text-left mb-10">
              <div className="flex flex-row items-start mb-5">
                ...
                <div className="bg-gray-100 p-5 px-10 w-full flex items-center dark:bg-gray-900">
                  <h4 className="text-md leading-6 font-medium text-gray-900 dark:text-gray-500">{faq.question}</h4>
                </div>
              </div>
